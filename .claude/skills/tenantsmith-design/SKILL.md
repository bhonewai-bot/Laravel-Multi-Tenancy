...
| -------- | --------- | ----------------------- | ------------------------------------- |
| Page bg  | `#08080c` | `dark:bg-[#08080c]`     | Body, page background                 |
| Surface  | `#101016` | `dark:bg-[#101016]`     | Cards, sidebar, header, inputs        |
| Elevated | `#181820` | `dark:bg-[#181820]`     | Hover states, dropdowns, active items |
| Border   | `#262632` | `dark:border-[#262632]` | All borders, input borders            |
...
bg-white dark:bg-[#101016]           {{-- cards, sidebar, header --}}
bg-gray-100 dark:bg-[#08080c]        {{-- page background --}}
bg-gray-50 dark:bg-[#181820]         {{-- hover, elevated --}}
...
border-gray-200 dark:border-[#262632]

{{-- Interactive --}}
hover:bg-gray-100 dark:hover:bg-[#181820]
hover:text-gray-900 dark:hover:text-gray-100
focus:ring-brand-500 focus:ring-offset-2 dark:focus:ring-offset-[#08080c]
```
...
The `auth-input` component uses a floating-label pattern with Alpine.js. Inputs use a **refined border** with subtle depth.
...
<div x-data="{ focused: false, value: '{{ old($name) }}', showPassword: false }" class="relative">
    ...
    <input ... class="block w-full pt-5 pb-2 px-3 text-sm rounded-lg
...
- Border: `border border-gray-200 dark:border-[#262632]`
- Border radius: `rounded-lg`
- Padding: `pt-5 pb-2 px-3` (floating label needs top padding)
- Font size: `text-sm`
- Shadow: `shadow-card` default, `shadow-glow-brand` on focus
- Background: `bg-white dark:bg-[#101016]`
...
**Section label:** `text-[10px] font-semibold uppercase tracking-[0.08em] text-gray-400 dark:text-gray-500` — hidden when collapsed via `x-show="!$store.sidebar.collapsed" x-cloak`.
...
```blade
<a href="..." class="flex items-center gap-2.5 rounded-lg px-3 py-1.5 text-sm transition-colors
    hover:bg-gray-100 dark:hover:bg-[#181820]
    {{ $active ? 'bg-gray-100 dark:bg-[#181820] text-gray-900 dark:text-gray-100 font-medium' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100' }}"
    :title="$store.sidebar.collapsed ? 'Label' : ''">
    <x-heroicon-o-xxx class="w-5 h-5 shrink-0 {{ $active ? 'text-brand-500 dark:text-brand-400' : 'text-gray-400 dark:text-gray-500' }}" />
...
- Sidebar width: `w-56` (expanded) / `w-16` (collapsed), persisted in `localStorage`
...
- Right panel: form area, `bg-white dark:bg-[#08080c]`
...
- **Using `dark:bg-gray-800` instead of `dark:bg-[#101016]`** — the dark theme uses custom hex values, not standard Tailwind grays.
- **Missing `dark:` variant** — the most common mistake. Every `bg-white` needs `dark:bg-[#101016]`.
- **Using raw `indigo-*` instead of `brand-*`** — the palette is aliased so it can be swapped later.
- **Forgetting `dark:focus:ring-offset-[#08080c]`** on buttons — the focus ring offset must match the dark page background.
- **Using `shadow-sm` on inputs/buttons** — use `shadow-card` for default, `shadow-glow-brand` on focus/hover. The design uses CSS-variable-driven shadows, not Tailwind's built-in `shadow-sm`.
...
- **`:class` on Blade components (`<x-*>`) is parsed as PHP, not Alpine.js** — Blade's component tag compiler treats `:class` as a PHP prop binding. On `<x-heroicon-o-chevron-down :class="open ? 'rotate-180' : ''" />`, `open` is evaluated as a PHP constant and throws "Undefined constant". **Fix:** use `x-bind:class` instead of `:class` on any `<x-*>` component. On plain HTML elements (`<div>`, `<span>`, `<a>`), `:class` works fine for Alpine.js.
