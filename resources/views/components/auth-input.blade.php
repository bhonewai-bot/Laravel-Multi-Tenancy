@props(['name', 'type' => 'text', 'label', 'required' => false, 'autofocus' => false, 'autocomplete' => 'off'])

@php
    $hasError = isset($errors) && $errors->has($name);
@endphp

<div
    x-data="{ focused: false, value: '{{ old($name) }}', showPassword: false }"
    class="relative"
>
    {{-- Floating label --}}
    <label
        for="{{ $name }}"
        class="absolute left-3 transition-all duration-200 pointer-events-none text-sm
            {{ $hasError ? 'text-red-500 dark:text-red-400' : '' }}
            top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500"
        :class="{
            'top-2 !translate-y-0 text-xs font-medium': focused || value,
            {{ $hasError ? "'text-red-500 dark:text-red-400': focused || value" : "'text-brand-500 dark:text-brand-400': focused || value" }}
        }"
    >
        {{ $label }}
    </label>

    {{-- Input --}}
    <input
        id="{{ $name }}"
        name="{{ $name }}"
        :type="{{ $type === 'password' ? 'showPassword ? \'text\' : \'password\'' : "'{$type}'" }}"
        x-model="value"
        @focus="focused = true"
        @blur="focused = false"
        @disabled(false)
        {{ $required ? 'required' : '' }}
        {{ $autofocus ? 'autofocus' : '' }}
        autocomplete="{{ $autocomplete }}"
        {{ $attributes->merge(['class' => '
            block w-full pt-5 pb-2 px-3 text-sm rounded-lg
            border border-gray-200 dark:border-[#262632]
            bg-white dark:bg-[#101016]
            text-gray-900 dark:text-gray-100
            shadow-card focus:shadow-glow-brand
            focus:border-brand-500 focus:ring-2 focus:ring-brand-500/30 dark:focus:ring-brand-400/20
            transition-all duration-200
            ' . ($hasError
                ? 'border-red-400 dark:border-red-500 focus:border-red-500 focus:ring-red-500/30'
                : ''
            ) . '
        ']) }}
    />

    {{-- Password toggle --}}
    @if ($type === 'password')
        <button
            type="button"
            @click="showPassword = !showPassword"
            class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors focus:outline-none"
            tabindex="-1"
        >
            {{-- Eye open --}}
            <svg x-show="!showPassword" x-cloak class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
            {{-- Eye closed --}}
            <svg x-show="showPassword" x-cloak class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88" />
            </svg>
        </button>
    @endif

    {{-- Error message --}}
    @if ($hasError)
        <p class="mt-1.5 text-xs text-red-600 dark:text-red-400 animate-shake">
            {{ $errors->first($name) }}
        </p>
    @endif
</div>
