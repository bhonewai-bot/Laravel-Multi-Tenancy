    ...
    <div class="animate-fade-up">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $users->total() }} user{{ $users->total() === 1 ? '' : 's' }}</p>
            </div>
        ...
        @if ($users->isEmpty())
            ...
            <div class="rounded-xl border border-gray-200 dark:border-[#262632] bg-white dark:bg-[#101016] p-12 text-center">
                <div class="mx-auto w-12 h-12 rounded-full bg-brand-50 dark:bg-brand-500/10 flex items-center justify-center mb-4">
                    <x-heroicon-o-users class="w-6 h-6 text-brand-600 dark:text-brand-400" />
                </div>
                <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-1">No users yet</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Get started by adding your first user to this tenant.</p>
                @can('create', App\Models\User::class)
                    <a href="{{ route('tenant.users.create', absolute: false) }}">
        ...
        @else
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-200 dark:border-[#262632] bg-gray-50/50 dark:bg-[#0e0e15]/50">
                                <th class="px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">User</th>
                                <th class="px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Email</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-[#181820]">
                            @foreach ($users as $user)
                                <tr class="group hover:bg-gray-50/70 dark:hover:bg-[#181820]/70 transition-all duration-150">
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-9 h-9 rounded-lg bg-brand-50 dark:bg-brand-500/10 border border-brand-100 dark:border-brand-500/20 flex items-center justify-center shrink-0">
                                                <span class="text-sm font-semibold text-brand-600 dark:text-brand-400">{{ strtoupper(substr($user->name ?? 'U', 0, 1)) }}</span>
                                            </div>
                                            <div class="min-w-0">
                                    ...
                                    {{-- Created --}}
                                    <td class="px-5 py-4">
                                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ optional($user->created_at)->format('M d, Y') }}</span>
                                    </td>
                    ...
                    @foreach ($users as $user)
                        <div class="p-4 hover:bg-gray-50/50 dark:hover:bg-[#181820]/50 transition-colors duration-150">
                            ...
                            <div class="flex items-center gap-4 text-sm mb-3">
                                <span class="text-gray-500 dark:text-gray-400">{{ optional($user->created_at)->format('M d, Y') }}</span>
                            </div>

                            <div class="flex items-center gap-2">
                                <a href="{{ route('tenant.users.show', $user, absolute: false) }}"
            ...
            @foreach ($users as $user)
                @can('delete', $user)
                    <x-delete-modal name="delete-user-{{ $user->id }}" :action="route('tenant.users.destroy', $user, absolute: false)" entity="user" />
                @endcan
            @endforeach
